...
- Alternatively:
  ...
  variableName.close()
  ```
- Once the file is open (where `variableName` is a file object), you have two main ways to process it:
    - Loop through it line by line using `for line in variableName`, which reads one line at a time as a string and allows you to process or append it to a list.
    - Or use `variableName.read().split()`, which reads the entire file into a single string and splits it into a list of strings using whitespace by default (spaces, tabs, or newlines). `"1 hello".split()` → `["1", "hello"]`  
    
      **Note:** In `words.txt`, each word is on a new line, so both approaches will produce the same result.
    
    
- Words may include extra whitespace or newline characters. Use `.strip()` to remove surrounding whitespace.
